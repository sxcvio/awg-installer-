...
ListenPort = $VPN_PORT
# DNS намеренно отсутствует: на сервере awg-quick DNS= перезаписывает системный
# резолвер через resolvconf и ломает DNS для всех процессов (в т.ч. бота).
# DNS нужен только в клиентских конфигах — бот добавляет его туда сам.
